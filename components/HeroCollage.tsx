...
const SLOT_FRACTIONS_MOBILE: [number, number][] = [
  [-0.26, -0.31],  // left-top (small)
  [-0.26,  0.16],  // left-bottom (small)
  [ 0.15, -0.07],  // right (large/active home)
  [ 0,     0   ],  // unused (hidden)
  [ 0,     0   ],  // unused (hidden)
]
// Where the active (large) card sits: centre on desktop, shifted right + up on mobile.
const ACTIVE_POS_DESKTOP: [number, number] = [0, 0]
const ACTIVE_POS_MOBILE:  [number, number] = [0.15, -0.07]

const ACTIVE_SCALE          = 1.45  // front card prominent (desktop)
const ACTIVE_SCALE_MOBILE   = 1.32  // big standout card on the right (mobile)
